...
const insightService_1 = require("../src/services/insightService");
const Insight_1 = require("../src/entities/Insight");
const Transaction_1 = require("../src/entities/Transaction");
jest.mock("../src/config/db", () => ({
    AppDataSource: {
        getRepository: jest.fn().mockImplementation((entity) => {
            if (entity === Insight_1.Insight)
                return mockInsightRepo;
            if (entity === Transaction_1.Transaction)
...
describe("insightService.compute", () => {
    ...
    it("should compute insights correctly", async () => {
        ...
        const result = await (0, insightService_1.compute)(1);
    ...
    it("should throw error when no transactions found", async () => {
        mockTransactionRepo.find.mockResolvedValue([]);
        await expect((0, insightService_1.compute)(1)).rejects.toThrow("No transactions found");
    });
});
